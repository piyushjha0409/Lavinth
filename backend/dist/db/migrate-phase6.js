...
function migrate() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            logger_1.default.info("Running Phase 6 migration: Threat Intelligence...");
            // Create tables and alter existing
            yield pool.query(phase6Schema);
            logger_1.default.info("Phase 6 tables created / altered successfully");
            // Seed sources
            yield pool.query(seedSources);
            logger_1.default.info("Threat intel sources seeded");
            // Clean up stale sources and ensure new ones exist
            // Delete sync logs first (FK constraint), then stale sources
            yield pool.query(`DELETE FROM threat_intel_sync_log WHERE source_id IN ('blowfish', 'solana-safety-101', 'haveibeendrained')`);
            yield pool.query(`DELETE FROM threat_intel_sources WHERE source_id IN ('blowfish', 'solana-safety-101', 'haveibeendrained')`);
            yield pool.query(`
      INSERT INTO threat_intel_sources (source_id, source_name, source_type, source_url, is_enabled)
      VALUES
        ('allenhark', 'AllenHark Scammer Database', 'community_list', 'https://allenhark.com/blacklist.jsonl', true),
      ...
      ON CONFLICT (source_id) DO NOTHING
    `);
            logger_1.default.info("Stale sources removed, new sources ensured");
            // Verify
            const sources = yield pool.query("SELECT source_id, source_name, source_type FROM threat_intel_sources");
            logger_1.default.info({ count: sources.rows.length }, 'Verified threat intel sources');
            sources.rows.forEach((row) => {
                logger_1.default.info({ sourceId: row.source_id, sourceName: row.source_name, sourceType: row.source_type }, 'Threat intel source');
            });
            const cols = yield pool.query(`
    ...
    `);
            logger_1.default.info({ count: cols.rows.length, expected: 3 }, 'Verified new columns on known_malicious_delegates');
            logger_1.default.info("Phase 6 migration completed successfully!");
        }
        catch (error) {
            logger_1.default.error({ err: error }, 'Phase 6 migration failed');
            throw error;
        }
        finally {
            yield pool.end();
