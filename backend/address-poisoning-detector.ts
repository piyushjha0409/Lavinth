...
export interface AddressClassification {
  ...
  legitimacyScore: number; // 0-1 where 1 is definitely legitimate
...
const MIN_TRANSACTION_COUNT = 3; // Minimum transactions to establish a pattern
...
export class AddressPoisoningDetector {
  ...
  public async getAddressInfo(address: string): Promise<AddressInfo> {
    ...
    try {
      ...
      const addressInfo: AddressInfo = {
        address,
        firstSeen: result.rows[0]?.first_seen ? new Date(result.rows[0].first_seen).getTime() : Date.now(),
        lastSeen: result.rows[0]?.last_seen ? new Date(result.rows[0].last_seen).getTime() : Date.now(),
        incomingTransactionCount: parseInt(result.rows[0]?.incoming_count || '0'),
        outgoingTransactionCount: parseInt(result.rows[0]?.outgoing_count || '0'),
        totalTransactionVolume: parseFloat(result.rows[0]?.total_volume || '0'),
        isLabeled: result.rows[0]?.is_labeled || false,
   ...
   */
  public async findSimilarAddresses(address: string, userAddresses: string[]): Promise<SimilarityResult[]> {
    // Check cache first
   ...
   */
  public async groupSimilarAddresses(addresses: string[]): Promise<AddressGroup[]> {
    ...
    for (const address of addresses) {
      ...
      const similarAddresses = await this.findSimilarAddresses(address, addresses);
      if (similarAddresses.length > 0) {
        // Create a group with this address and all similar addresses
        const groupAddresses = [address, ...similarAddresses.map(s => s.address2)];
        ...
        const addressInfos = await Promise.all(groupAddresses.map(addr => this.getAddressInfo(addr)));
        ...
        for (let i = 0; i < groupAddresses.length; i++) {
          for (let j = i + 1; j < groupAddresses.length; j++) {
            const similarity = this.calculateAddressSimilarity(groupAddresses[i], groupAddresses[j]);
        ...
        const isPotentialPoisoningGroup = this.assessGroupPoisoningLikelihood(addressInfos);
        ...
        groups.push({
          ...
          newestAddress,
          averageSimilarity,
          isPotentialPoisoningGroup: isPotentialPoisoningGroup.isPotentialPoisoning,
   ...
   */
  private assessGroupPoisoningLikelihood(addressInfos: AddressInfo[]): { isPotentialPoisoning: boolean, confidence: number } {
    ...
    for (const addr of newerAddresses) {
      if (oldestAddress.totalTransactionVolume > 10 * addr.totalTransactionVolume) {
        poisoningFactors++;
      }
      maxFactors++;
    }
    ...
    for (const addr of newerAddresses) {
      const oldestHasBidirectional = oldestAddress.incomingTransactionCount > 0 && 
                                   oldestAddress.outgoingTransactionCount > 0;
      const newerHasBidirectional = addr.incomingTransactionCount > 0 && 
                                  addr.outgoingTransactionCount > 0;
    ...
    const poisoningLikelihood = maxFactors > 0 ? poisoningFactors / maxFactors : 0;
    
    // Calculate confidence based on amount of data
    const totalTransactions = addressInfos.reduce(
      (sum, addr) => sum + addr.incomingTransactionCount + addr.outgoingTransactionCount, 
      0
    );
   ...
   */
  private calculateVisualSimilarity(address1: string, address2: string): number {
   ...
   */
  private calculatePrefixSimilarity(address1: string, address2: string): number {
    ...
    let suffixLength = 0;
    for (let i = 0; i < minLength; i++) {
      if (address1[address1.length - 1 - i] === address2[address2.length - 1 - i]) {
        suffixLength++;
      } else {
        break;
      }
   ...
   */
  private calculateKeyboardSimilarity(address1: string, address2: string): number {
    if (address1.length !== address2.length) return 0;
    ...
    for (let i = 0; i < address1.length; i++) {
      ...
      if (char1 === char2) {
        adjacencyMatches++;
      } else if (KEYBOARD_ADJACENCY_MAP[char1] && KEYBOARD_ADJACENCY_MAP[char1].includes(char2)) {
   ...
   */
  public calculateAddressSimilarity(address1: string, address2: string): SimilarityResult {
    // Calculate different similarity metrics
    const visualSimilarity = this.calculateVisualSimilarity(address1, address2);
    const levenshteinSimilarity = 1 - (this.levenshteinDistance(address1, address2) / Math.max(address1.length, address2.length));
   ...
   */
  public async classifyAddress(address: string, userAddresses: string[]): Promise<AddressClassification> {
    // Get address info
    const addressInfo = await this.getAddressInfo(address);
    
    // Find similar addresses
    const similarAddresses = await this.findSimilarAddresses(address, userAddresses);
    ...
    const hasBidirectionalHistory = addressInfo.incomingTransactionCount > 0 && 
    ...
    if (hasBidirectionalHistory) {
      ...
      legitimacyScore += 0.1;
    } else if (addressInfo.incomingTransactionCount > 0 && addressInfo.outgoingTransactionCount === 0) {
    ...
    if (addressInfo.incomingTransactionCount + addressInfo.outgoingTransactionCount >= MIN_TRANSACTION_COUNT) {
    ...
    // Calculate confidence based on amount of data
    const transactionCount = addressInfo.incomingTransactionCount + addressInfo.outgoingTransactionCount;
  ...
  private isAddressLikelyLegitimate(addressInfo: AddressInfo): boolean {
    ...
    // Addresses with substantial transaction volume are more likely legitimate
    const hasSubstantialVolume = addressInfo.totalTransactionVolume > DUST_THRESHOLD * 100;
    
    // Addresses with many transactions are more likely legitimate
    const hasMultipleTransactions = addressInfo.incomingTransactionCount + 
                                  addressInfo.outgoingTransactionCount >= MIN_TRANSACTION_COUNT;
   ...
   */
  public async validateTransactionAddress(selectedAddress: string, userHistory: string[]): Promise<{
    isValid: boolean;
    ...
    const classification = await this.classifyAddress(selectedAddress, userHistory);
